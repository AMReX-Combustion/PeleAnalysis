...
int
NumElemXinSpecY(const std::string& elem, const std::string& spec)
{
...
int
NumElemXinSpecY(int elem, int spec)
{
  int nelements = mech_util::NumElements();
  int nspecies = mech_util::NumSpecies();
...
Edge::Edge(
  const std::string& n1, const std::string& n2, int reac, amrex::Real weight)
  : sp1(n1), sp2(n2)
{
...
int
Edge::equivSign(const Edge& rhs) const
{
...
void
Edge::combine(const Edge& rhs, const int sgn)
{
  if (sgn != 0) {
    ...
    for (int i = 0; i < delSize; ++i) {
      RWL[oldSize + i] =
        std::pair<int, amrex::Real>(rhs.RWL[i].first, sgn * rhs.RWL[i].second);
    }
  }
}
...
{
  os << e.sp1 << " <=> " << e.sp2 << "  ( ";
  for (amrex::Vector<std::pair<int, amrex::Real>>::const_iterator it =
         e.RWL.begin();
       it != e.RWL.end(); ++it) {
...
Group::Group(const std::map<std::string, int>& eltCnts) { mEltCnts = eltCnts; }

Group::Group(const Group& rhs) { mEltCnts = rhs.mEltCnts; }

Group
Group::operator-(const Group& rhs) const
{
  Group val(*this);
  for (std::map<std::string, int>::const_iterator rit = rhs.mEltCnts.begin();
       rit != rhs.mEltCnts.end(); ++rit) {
...
{
  Group val(*this);
  for (std::map<std::string, int>::iterator vit = val.mEltCnts.begin();
       vit != val.mEltCnts.end(); ++vit) {
...
Group::awt() const
{
  ...
  amrex::Real myWt = 0;
  for (std::map<std::string, int>::const_iterator it = mEltCnts.begin();
       it != mEltCnts.end(); ++it) {
...
{
  os << "Group < ";
  for (std::map<std::string, int>::const_iterator it = g.mEltCnts.begin();
       it != g.mEltCnts.end(); ++it) {
    os << it->first << ":" << it->second << " ";
  }
  os << ">";
  return os;
...
getEdges(const std::string& trElt, int PrintVerbose, int HackSplitting)
{
  ...
  for (int r = 0; r < NumReactions(); ++r) {
    const amrex::Vector<std::pair<std::string, int>>& coeffs =
      specCoeffsInReactions(r);
    ...
    for (std::map<std::string, int>::const_iterator it = net.begin();
         it != net.end(); ++it) {
    ...
    if ((LR == 1) || (LP == 1)) { // exactly one tr-containing species either
                                  // side
      for (std::map<std::string, int>::const_iterator rit = reac.begin();
           rit != reac.end(); ++rit) {
        const std::string& spcr = rit->first;
        const int cor = rit->second;

        for (std::map<std::string, int>::const_iterator pit = prod.begin();
             pit != prod.end(); ++pit) {
          const std::string& spcp = pit->first;
          const int cop = pit->second;
          int w =
            std::min(cor * groups[spcr][trElt], cop * groups[spcp][trElt]);
    ...
    if ((LR == 2) && (LP == 2)) { // two tr-containing species each side
      ...
      std::map<std::string, int>::const_iterator r1 = r0;
      r1++;
      std::map<std::string, int>::const_iterator p0 = prod.begin();
      std::map<std::string, int>::const_iterator p1 = p0;
      p1++;
    ...
    Print() << "Cannot decompose rxn: " << r << " " << LR << " " << LP
            << std::endl;
  }
  ...
  while (edges.size() != 0) {
    const Edge oe(edges.front());
    edges.pop_front();
    bool foundIt = false;
    // There's probably a faster way to search this list...
    for (std::list<Edge>::iterator it = uEdges.begin();
         !foundIt && it != uEdges.end(); ++it) {
