      ...
      char* argv[])
{
  Initialize(argc,argv);
  {
    ...
    Vector<std::unique_ptr<MultiFab>> indata(1);
    MultiFab outdata;
    ...
    std::vector<std::string> plt_files; pp.getarr("plt_files",plt_files);
    ...
    //  starting the loop over all plt files
    for(int ifile=0;ifile<number_of_plts;ifile++){
      plotFileName = plt_files[ifile];
      ...
      if(ifile == number_of_plts-1){
          Geometry geom(amrData.ProbDomain()[Nlev-1],&rb,coord,&(is_per[0]));
          
          for (MFIter mfi(*indata[Nlev-1],TilingIfNotGPU()); mfi.isValid(); ++mfi)
          {
            const Box& box = mfi.tilebox();
            amrex::Array4<Real> const output = outdata.array(mfi);
            ...
            AMREX_PARALLEL_FOR_3D ( box, i, j, k,
            {
              ...
              for (int n=0; n<amrData.PlotVarNames().size(); ++n){
                 output(i,j,k,n) = output(i,j,k,n)/number_of_plts;
              }
      
            });
          }

        std::string outfile("plt_ensemble");
