...
if __name__ == '__main__':
    ...
    dim_consts.update({
        ...
        'temp_var_factor':      (gamma - 1) * dim_consts['T0'] * dim_consts['Mach']**2,
        'dilatation_factor':    dim_consts['urms0'] / dim_consts['lambda0']
        })

    ## Load data, initialize storage
    ts = yt.load(args.root_dir + '/plt[0-9]*[0-9]')
    storage = {}

    ## Disambiguate Fields
    temp_field = find_temp(ts[0].field_list)
    ...
    for store, ds in ts.piter(storage=storage):
        ...
        # Temperature fields
        temp_avg = ds.all_data().quantities.weighted_average_quantity(temp_field, 'cell_volume')
        ds.add_field(
                ('boxlib', 'temp_var'),
                units = 'K',
                function = get_temp_var_func(temp_avg, temp_field),
                sampling_type = 'cell'
                )
        ds.add_field(
                ('boxlib', 'temp_var_sq'),
                units = 'K**2',
                function = get_temp_var_sq_func(temp_avg, temp_field),
                sampling_type = 'cell'
                )
        ...
        results.update({
            ...
            'urms_computed':  np.sqrt(results['kin_energy_avg']/3) ,
            'Mach_t_computed':  np.sqrt(results['kin_energy_avg']) / dim_consts['c_s0'] ,
            'magvort_sq_avg_adim':    results['magvort_sq_avg'] /
                (dim_consts['urms0'] / dim_consts['lambda0'])**2,
            'temp_var_sq_avg_adim':        results['temp_var_sq_avg'] / ((gamma - 1) *
                dim_consts['T0'] * dim_consts['Mach']**2)**2,
    ...
    tseries_plots = {
            ...
            'urms_computed':  {
                'x' : stats['time_adim'],
                'y' : stats['urms_computed'],
                'xlabel' : r'Time (\( t / \tau \))' ,
                'ylabel' : r'\(  u_\mathrm{rms} \)',
                'title' : 'computed urms from kinetic energy'
                },
            'Mach_t_computed':  {
                'x' : stats['time_adim'],
                'y' : stats['Mach_t_computed'],
                'xlabel' : r'Time (\( t / \tau \))' ,
                'ylabel' : r'\(    \)',
                'title' : 'computed Mach_t from kinetic energy'
                },
            'magvort_sq_avg_adim':  {
                'x' : stats['time_adim'],
            ...
            'temp_var_sq_avg_adim': {
                'x' : stats['time_adim'],
                'y' : stats['temp_var_sq_avg_adim'],
                'xlabel' : r'Time (\( t / \tau \))' ,
                'ylabel' : r'\( \mathbb{E}\left[(T^\prime)^2\right] / ((\gamma - 1) T_0 M^2_{t_0})^2 \)',
                'title' : 'Temperature'
                },
            'divu_sq_avg_adim' : {
                'x' : stats['time_adim'],
